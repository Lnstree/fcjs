...
//        return APU::access<wr>(elapsed(), addr, v);
        ...
        if (wr) { Joypad::write_strobe(v & 1); }     // Joypad strobe.
            else return Joypad::read_state(0);      // Joypad strobe.
    }
...
inline u16 rd16_d(u16 a, u16 b) { 
	u8 aa = rd(a);
	u8 bb = rd(b);
	return aa | (bb << 8); }  // Read from A and B and merge.
...
template<Mode m> void SBC() { G ^ 0xFF; 
		s16 r = A + p + P[C]; 
		upd_cv(A, p, r); 
		upd_nz(A = r); }
template<Mode m> void BIT() { G;
		P[Z] = !(A & p);
		P[N] = p & 0x80; 
		P[V] = p & 0x40; }
...
template<IntType t> void INT()
{
    ...
    P[I] = true;
                          /*   NMI    Reset    IRQ     BRK  */
    const u16 vect[] = { 0xFFFA, 0xFFFC, 0xFFFE, 0xFFFE };
    PC = rd16(vect[t]);
