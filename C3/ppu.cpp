...
u8 rd(u16 addr)
{
    if (addr <= 0x1FFF){
		...
		u8 temp = cgRam[addr & 0x1F] & (mask.gray ? 0x30 : 0xFF);
...
template <bool write> u8 access(u16 index, u8 v)
{
    ...
    static bool latch;  // Detect second readin.
//	qDebug() << "b:" << write << index << v << res << status.r;
    /* Write into register */
    if (write)
    {
        res = v;
...
//    qDebug() << "a:" << write << index << v <<  res << status.r;
    return res;
}
...
void pixel()
{
    ...
    if (scanline < 240 && x >= 0 && x < 256)
    {
		...
		int tt = rd(0x3F00 + (rendering() ? palette : 0));
		//qDebug() << tt;
        pixels[scanline*256 + x] = nesRgb[tt];
    }
...
template<Scanline s> void scanline_cycle()
{
    ...
    else if (s == POST &&  dot == 0); //NesWindow::setFrame(pixels); //GUI::new_frame(pixels);
    else if (s == VISIBLE || s == PRE)
    {
        ...
        if ((dot >= 2 && dot <= 255) || (dot >= 322 && dot <= 337)){
            ...
            pixel(); reload_shift(); h_update();
        }
        else if (dot >= 280 && dot <= 304){
            if (s == PRE)            v_update();
        }else if (dot == 1){
            addr = nt_addr(); if (s == PRE) status.vBlank = false;
        }else if (dot == 321 || dot == 339){
            addr = nt_addr();
        }else if (dot == 338){
            nt = rd(addr);
        }else if (dot == 340){
            nt = rd(addr); if (s == PRE && rendering() && frameOdd) dot++;
        }
